...
mat2 r(float p){return mat2(cos(p),-sin(p),sin(p),cos(p));}\
float n(vec2 p){return sin(1.5*p.x)*sin(1.5*p.y);}\
const mat2 m2 = mat2(0.8,-0.6,0.6,0.8);\
float b(vec2 p){float f=0.;f+=0.5*n(p);p=m2*p*2.02;f+=.25*n(p);p=m2*p*2.03;f+=.125*n(p);p=m2*p*2.01;f+=.0625*n(p);return f/.9375;}\
float c(in vec2 d) {\
